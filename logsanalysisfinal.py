...
GROUPBY articles.title ORDERBY views DESC limit 3;"""

# Question 2: Who are the most popular authors by view?
Query2 = """ SELECT authors.name, COUNT(*) as views from articles
INNER JOIN authors on articles.author = authors.id INNER JOIN log
on log.path LIKE CONCAT('/article/', articles.slug) WHERE log.status
